...
'''Model'''
class DirectVoxGOVonMises(torch.nn.Module):
    def __init__(self, xyz_min, xyz_max,
                 ...
                 viewbase_pe=4,
                 **kwargs):
        super(DirectVoxGOVonMises, self).__init__()
        ...
        print('dvgo_vm: set density bias shift to', self.act_shift)
        ...
        if rgbnet_dim <= 0:
            ...
            print('dvgo_vm: feature voxel grid', self.k0)
            print('dvgo_vm: mlp', self.rgbnet)
    ...
    def _set_grid_resolution(self, num_voxels):
        ...
        print('dvgo_vm: voxel_size      ', self.voxel_size)
        print('dvgo_vm: world_size      ', self.world_size)
        print('dvgo_vm: voxel_size_base ', self.voxel_size_base)
        print('dvgo_vm: voxel_size_ratio', self.voxel_size_ratio)
    ...
    def scale_volume_grid(self, num_voxels):
        print('dvgo_vm: scale_volume_grid start')
        ori_world_size = self.world_size
        self._set_grid_resolution(num_voxels)
        print('dvgo_vm: scale_volume_grid scale world_size from', ori_world_size.tolist(), 'to', self.world_size.tolist())
        ...
        print('dvgo_vm: scale_volume_grid finish')
    ...
    def voxel_count_views(self, rays_o_tr, rays_d_tr, imsz, near, far, stepsize, downrate=1, irregular_shape=False):
        print('dvgo_vm: voxel_count_views start')
        ...
        print('dvgo_vm: voxel_count_views finish (eps time:', eps_time, 'sec)')
        return count
    ...
    def forward(self, rays_o, rays_d, viewdirs, global_step=None, **render_kwargs):
        ...
        if self.rgbnet is None:
            ...
            rgbk = torch.zeros_like(k0)
            rgbk[:, :3] = torch.sigmoid(k0[:, :3])
            rgbk[:, 3] = F.leaky_relu(k0[:, 3])
            # rgbk = k0
        else:
            ...
            # kappa prediction
            kappa_logit = self.kappanet(rgb_feat)

            if self.rgbnet_direct:
                rgb = torch.sigmoid(rgb_logit)
                kappa = F.leaky_relu(kappa_logit)
                rgbk = torch.cat([rgb, kappa], dim=-1)
            else:
                rgb = torch.sigmoid(rgb_logit + k0_diffuse)
                print("kappa unimplemented here!")
        # Ray marching
        rgb_marched = segment_coo(
                src=(weights.unsqueeze(-1) * rgbk),
                index=ray_id,
                out=torch.zeros([N, 4]),
                reduce='sum')
        breakpoint()
        ...
        #         upper_thres = weights > 0.1
        #         upper_ray_id = ray_id[upper_thres]
        #         upper_s = step_id[upper_thres]
                
        #         prev_ray_id = -1
        #         for id, step_id in zip(upper_ray_id.tolist(), upper_s.tolist()):
        #             if id == prev_ray_id:
        #                 continue
        #             else:
        #                 prev_ray_id = id
