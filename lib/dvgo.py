...
from torch_scatter import segment_coo, scatter_max
...
class DirectVoxGO(torch.nn.Module):
    ...
    def sample_ray(self, rays_o, rays_d, near, far, stepsize, **render_kwargs):
        ...
        return ray_pts, ray_id, step_id, t_min, t_max, stepdist
    ...
    def forward(self, rays_o, rays_d, viewdirs, global_step=None, **render_kwargs):
        ...
        ray_pts, ray_id, step_id, t_min, t_max, stepdist = self.sample_ray(
        ...
        if render_kwargs['white_bkgd']:
            rgb_marched += (torch.stack([alphainv_last] * 3, dim=-1) * torch.tensor(render_kwargs['bg_color']))
        ...
        if render_kwargs.get('render_depth', False):
            # add sigma thresholding for depth rendering
            thres = render_kwargs.get('render_depth_sigma_thres', 0)
            if thres > 0:
                n_step = -torch.ones(N, dtype=torch.int64)
                above_thresh_ray_id = ray_id[density > thres]
                above_thresh_step_id = step_id[density > thres]
                n_step[above_thresh_ray_id] = above_thresh_step_id
                depth = t_min + stepdist * n_step
                depth[n_step == -1] = t_max[0]
            else:
                # weights[step_id < 500] = 0
                with torch.no_grad():
                    n_step = segment_coo(
                            src=(weights * step_id),
                            index=ray_id,
                ...
                # original code
                depth = t_min + stepdist * n_step
                # no sigma region to back
                # max_n_step = (t_max - t_min) // stepdist
                # depth = t_max + stepdist * (max_n_step - n_step)
...
def get_rays(H, W, K, c2w, inverse_y, flip_x, flip_y, mode='center'):
    # if K == -1:
    #     dirs = get_ray_directions_360(H, W)
    # else:
    i, j = torch.meshgrid(
...
def get_ray_directions_360(H, W):
    # rays coordinates: [x,y,z]-[right,up,backward]
    i = torch.tile(torch.arange(W), (H,1)) + 0.5
    j = torch.tile(torch.arange(H), (W,1)).T + 0.5
    # the direction here is without +0.5 pixel centering as calibration is not so accurate
    # see https://github.com/bmild/nerf/issues/24
    
    phi = (1 - 2 * i / W) * np.pi       # longitude (pi, -pi)
    theta = (1 - 2 * j / H) * np.pi/2   # latitude (pi/2, -pi/2)
...
def get_training_rays_flatten(rgb_tr_ori, train_poses, HW, Ks, ndc, inverse_y, flip_x, flip_y):
    ...
    # N = (rgb_tr_ori != 0).any(3).sum() # number of foreground rays
    ...
    for c2w, img, (H, W), K in zip(train_poses, rgb_tr_ori, HW, Ks):
        ...
        # viewdirs = viewdirs.flatten(0,1)

        # fg_flag = (img != 0).any(1)
        # n = fg_flag.sum()
        # rgb_tr[top:top+n].copy_(img[fg_flag])
        # rays_o_tr[top:top+n].copy_(rays_o[fg_flag].to(DEVICE))
        # rays_d_tr[top:top+n].copy_(rays_d[fg_flag].to(DEVICE))
        # viewdirs_tr[top:top+n].copy_(viewdirs[fg_flag].to(DEVICE))
...
def get_training_rays_in_maskcache_sampling(rgb_tr_ori, train_poses, HW, Ks, ndc, inverse_y, flip_x, flip_y, model, render_kwargs):
    ...
    # rgb_tr = torch.zeros([N,3], device=DEVICE)
    ###########################################Changed################################################
    channels = rgb_tr_ori.shape[-1]
    rgb_tr = torch.zeros([N, channels], device=DEVICE)
    rays_o_tr = torch.zeros([N, 3])
    rays_d_tr = torch.zeros([N, 3])
    viewdirs_tr = torch.zeros([N, 3])
    ###########################################Changed################################################
    imsz = []
    top = 0
    for c2w, img, (H, W), K in zip(train_poses, rgb_tr_ori, HW, Ks):
