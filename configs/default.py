...
data = dict(
    ...
    imgdir='.',              # directory name of images
    ...
    use_depthmask = False,        # use mask to remove far region
    use_kappa = False,            # use test-time augmentation generated kappa
    use_mask = False,             # use mask to segment out bg pixels
    use_bernoulli = False,        # use test-time augmentation on mask(bernoulli)

    # bounding box config
    fixate_voxel = False,         # fix voxel size to real world points
    world_bbox = [0.3, -0.2, 0.0, 0.7, 0.2, 0.3], # world bounding box, [x1, y1, z1, x2, y2, z2]

    # bg groudn color
    mask_thresh = 0.5,
    bg_color = [0.5, 0.5, 1.0], # bg normal in color(0, 0, 1)
    # bg_color = [1.0, 1.0, 1.0], # bg color white(default)

    # render depth with sigma thresholding (Dex-NeRF)
    render_depth_sigma_thres=0.0,
...
coarse_train = dict(
    ...
    weight_bg_density=0.0,        # weight of background ray density regularization
    weight_obj_density=0.0,       # weight of object ray density regularization
...
coarse_model_and_render = dict(
    ...
    loss_type='mse',              # type of photometric loss (mse | von-mises)
)
