...
from data.dataloader_test import TestLoader
    ...
    with torch.no_grad():
        for data_dict in tqdm(test_loader):
            ...
            for b_idx in range(B):
                # save results
                img_name = data_dict['img_name'][b_idx]

                # 1. save input image
                img_vis = utils.unnormalize(img[b_idx, ...])

                target_path = '{}/{:06d}_img.png'.format(results_dir, cnt)
                plt.imsave(target_path, img_vis)

                # 2. predicted normal

                target_path = '{}/{:06d}-color_mu.png'.format(results_dir, cnt)
                plt.imsave(target_path, pred_norm_rgb[b_idx, :, :, :])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--imgs_dir', default='./exampless', type=str)
    parser.add_argument('--outdir', default='results', type=str)
    parser.add_argument('--batch_size', default=8, type=int)
    parser.add_argument('--checkpoint', default='experiments/nomask_noset1/models/checkpoint_iter_0000200000.pt', type=str)
    ...
    test_loader = TestLoader(args, args.imgs_dir).data
