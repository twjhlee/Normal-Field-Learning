...
from lib import utils, dvgo, dcvgo, dmpigo, dvgo_vm
...
def config_parser():
    ...
    parser.add_argument('--overwrite_datadir', default="None")
    parser.add_argument('--overwrite_logdir', default="None")
    ...
    parser.add_argument("--render_density_in_bbox", action='store_true')
...
def render_viewpoints(model, render_poses, HW, Ks, ndc, render_kwargs,
    ...
    for i, c2w in enumerate(tqdm(render_poses)):
        ...
        rgb = render_result['rgb_marched'][..., :3].cpu().numpy()
        # kappa = render_result['rgb_marched'][..., -1].cpu().numpy()
        ...
        if gt_imgs is not None and render_factor==0:
            p = -10. * np.log10(np.mean(np.square(rgb - gt_imgs[i][..., :3])))
            psnrs.append(p)
            if eval_ssim:
                ssims.append(utils.rgb_ssim(rgb, gt_imgs[i][..., :3], max_val=1))
            if eval_lpips_alex:
                lpips_alex.append(utils.rgb_lpips(rgb, gt_imgs[i][..., :3], net_name='alex', device=c2w.device))
            if eval_lpips_vgg:
                lpips_vgg.append(utils.rgb_lpips(rgb, gt_imgs[i][..., :3], net_name='vgg', device=c2w.device))
    ...
    if savedir is not None and dump_images:
        for i in trange(len(rgbs)):
            # Normalize
            # rgbs[i] = rgbs[i] * (1 - bgmaps[i]) + bgmaps[i]
            rgb8 = utils.to8b(rgbs[i])
            filename = os.path.join(savedir, '{:03d}.png'.format(i))
            imageio.imwrite(filename, rgb8)

            filename = os.path.join(savedir, 'depth_{:03d}.npy'.format(i))
            # TODO: get appropriate depth image
            # depth_img = depths[i] * model.voxel_size.item() * render_kwargs['stepsize'] + render_kwargs['near']
            # depth_img = depths[i] * (1 - bgmaps[i]) + bgmaps[i]
            depth_img = depths[i]
            np.save(filename, depth_img)
            plt.clf()
            plt.imshow(depth_img)
            # plt.clim(render_kwargs['near'], render_kwargs['far'])
            
            # Debug for DexNeRF
            # plt.clim(0.5, 3)

            plt.colorbar(shrink=0.85, pad=0)
            plt.savefig(filename.replace('.npy', '.png'), bbox_inches='tight', dpi=200)

            # Save bg_maps as well
            filename = os.path.join(savedir, 'bgmap_{:03d}.png'.format(i))
            imageio.imwrite(filename, (bgmaps[i].squeeze() * 255).astype(np.uint8))
...
def create_new_model(cfg, cfg_model, cfg_train, xyz_min, xyz_max, stage, coarse_ckpt_path):
    ...
    if cfg.coarse_model_and_render.loss_type in ['von-mises']:
        print(f'scene_rep_reconstruction ({stage}): \033[96muse dense voxel grid\033[0m')
        model = dvgo_vm.DirectVoxGOVonMises(
        ...
        model = dmpigo.DirectMPIGO(
            ...
            num_voxels=num_voxels,
            **model_kwargs)
    elif cfg.data.unbounded_inward:# or cfg.data.egocentric:
...
def load_existed_model(args, cfg, cfg_train, reload_ckpt_path):
    if cfg.coarse_model_and_render.loss_type in ['von-mises']:
        model_class = dvgo_vm.DirectVoxGOVonMises
    elif cfg.data.ndc:
...
def scene_rep_reconstruction(args, cfg, cfg_model, cfg_train, xyz_min, xyz_max, data_dict, stage, coarse_ckpt_path=None, is_eval=False):
    # init
    ...
    render_kwargs = {
        ...
        "white_bkgd": cfg.data.white_bkgd,
    }
    ...
    for global_step in trange(1+start, 1+cfg_train.N_iters):
        ...
        if global_step in cfg_train.pg_scale:
            ...
            if isinstance(model, (dvgo.DirectVoxGO, dcvgo.DirectContractedVoxGO, dvgo_vm.DirectVoxGOVonMises)):
        ...
        # bg mask by sampling target always 5 channel tensor [R,G,B, p_hat, kappa]
        if cfg_train.weight_bg_density > 0 and cfg.data.use_mask:
            p_hat = target[..., 3]
            # when p_hat is valid
            if cfg.data.use_bernoulli:
                rand_arr = torch.from_numpy(np.random.rand(len(target))).to(device)
                bg_mask = (rand_arr >= p_hat)
            # when p_hat is not valid
            else:
                bg_mask = (target[:, :3] == torch.Tensor(cfg.data.bg_color)).all(1)
        else:
            # no bg
            bg_mask = torch.zeros(len(target), dtype=torch.bool)
        if cfg.coarse_model_and_render.loss_type == 'mse':
            if cfg['data']['use_kappa']:
                # deprecated
                kappa = target[:, -1]
                loss = cfg_train.weight_main * \
                    F.mse_loss(
                        render_result['rgb_marched'][~bg_mask] * kappa[..., None][~bg_mask],
                        target[..., :3][~bg_mask] * kappa[..., None][~bg_mask]
                    )
            else:
                loss = cfg_train.weight_main * F.mse_loss(render_result['rgb_marched'][~bg_mask], target[..., :3][~bg_mask])
                mu = render_result['rgb_marched']
                # scale = torch.linalg.norm(mu * 2 - 1, dim=1)
                # scale_loss = torch.mean((scale - 1) ** 2)
                
            loss_mse = loss.detach()
        elif cfg.coarse_model_and_render.loss_type == 'angular':
            loss_mse = cfg_train.weight_main * F.mse_loss(render_result['rgb_marched'][:, :3][~bg_mask], target[:, :3][~bg_mask]).detach()

            mu = render_result['rgb_marched']
            dot = torch.cosine_similarity(mu * 2 - 1, target[: ,:3] * 2 - 1)
            kappa = target[..., -1]
            # maybe threshold kappa?
            kappa[kappa > 500] = 500

            dot = dot[~bg_mask]
            kappa = kappa[~bg_mask]

            # valid_mask = (dot.detach() < 0.999) * (dot.detach() > -0.999)
            # valid_mask = (dot.detach() < 0.999)
            # dot = dot[valid_mask]
            ...
            if cfg['data']['use_kappa']:
                loss = cfg_train.weight_main * torch.mean(-kappa * dot * 0.002)
            else:
                loss = cfg_train.weight_main * torch.mean(-dot)
        else:
            print("invalid loss: choose one of ['mse', 'von-mises', 'angular]")
            os._exit(0)

        psnr = utils.mse2psnr(loss_mse.detach())
        
        if cfg_train.weight_bg_density > 0 and cfg.data.use_mask:
            sigma_sum = segment_coo(
                src=render_result['raw_density'],
                index=render_result['ray_id'],
                out=torch.zeros([len(target)]),
                reduce='sum')
            sigma_sum_bg = sigma_sum[bg_mask]
            sigma_sum_obj = sigma_sum[~bg_mask]
            bg_loss = sigma_sum_bg.mean()
            obj_loss = sigma_sum_obj.mean()
            loss += cfg_train.weight_bg_density * bg_loss
            loss -= cfg_train.weight_obj_density * obj_loss
                # loss += scale_loss
        
        # debug code - when nan comes
        if torch.isnan(loss):
            breakpoint()
        ...
        if global_step%args.i_print==0:
            ...
            if cfg.coarse_model_and_render.loss_type == 'angular':
        ...
        # if global_step in [50000, 300000, 500000, 1000000]:
...
def train(args, cfg, data_dict):
    ...
    if cfg['data']['fixate_voxel']:
        xyz_min_coarse = np.array(cfg['data']['world_bbox'][0:3])
        xyz_max_coarse = np.array(cfg['data']['world_bbox'][3:6])
    ...
    if cfg.coarse_train.N_iters == 0:
        if cfg['data']['fixate_voxel']:
            xyz_min_fine, xyz_max_fine = xyz_min_coarse.copy(), xyz_max_coarse.copy()
        else:
            xyz_min_fine, xyz_max_fine = xyz_min_coarse.clone(), xyz_max_coarse.clone()
    elif cfg.coarse_model_and_render.loss_type in ['von_mises']:
        pass
        # TODO
    else:
        ...
        if cfg['data']['fixate_voxel']:
            xyz_min_fine = np.array(cfg['data']['world_bbox'][0:3])
            xyz_max_fine = np.array(cfg['data']['world_bbox'][3:6])
...
if __name__=='__main__':
    ...
    if args.overwrite_datadir != "None":
        cfg.data.datadir = args.overwrite_datadir
    if args.overwrite_datadir != "None":
        cfg.basedir = args.overwrite_logdir
    ...
    if args.export_fine_only:
        ...
        with torch.no_grad():
            ...
            if cfg.coarse_model_and_render.loss_type == 'von-mises':
                model_class = dvgo_vm.DirectVoxGOVonMises
            elif cfg.data.ndc:
                model_class = dmpigo.DirectMPIGO
            elif cfg.data.unbounded_inward:# or cfg.data.egocentric:
            ...
            alpha = model.activate_density(model.density.get_dense_grid()).squeeze().cpu().numpy()
            # rgb = torch.sigmoid(model.k0.get_dense_grid()).squeeze().permute(1,2,3,0).cpu().numpy()
            raw = model.k0.get_dense_grid().squeeze()
            rgb = torch.sigmoid(raw[:3]).permute(1,2,3,0).cpu().numpy()
            # TODO: if else on kappa
            kappa = F.relu(raw[-1]).cpu().numpy()
        savename = os.path.join(cfg['basedir'], cfg['expname'], "scene_mesh.npz")
        np.savez_compressed(savename, alpha=alpha, rgb=rgb, kappa=kappa)
        print('done')
        sys.exit()
    ...
    if args.render_test or args.render_train or args.render_video:
        if args.ft_path:
            ...
            if not os.path.exists(ckpt_path):
                ckpt_path = os.path.join(cfg.basedir, 'DVGO', 'fine_last.tar')
        ...
        if cfg.coarse_model_and_render.loss_type == 'von-mises':
            ...
            model_class = dvgo.DirectVoxGO
        model = utils.load_model(model_class, ckpt_path).to(device)
        if args.render_density_in_bbox:
            xyz_min, xyz_max = model.xyz_min.cpu().numpy(), model.xyz_max.cpu().numpy()
            # TODO: change this hard-coded line
            target_xyz_min, target_xyz_max = np.array(cfg['data']['world_bbox'][0:3]), np.array(cfg['data']['world_bbox'][3:6])

            # normalized coordinate of bbox
            bbox_diff_min = np.clip((target_xyz_min - xyz_min) / (xyz_max - xyz_min), 0. ,1.)
            bbox_diff_max = np.clip((target_xyz_max - xyz_min) / (xyz_max - xyz_min), 0., 1.)

            vox_min = (model.density.grid.shape[-3:] * bbox_diff_min).astype(int)
            vox_max = (model.density.grid.shape[-3:] * bbox_diff_max).astype(int)

            bbox_mask = torch.zeros_like(model.density.grid)
            bbox_mask[:, :, vox_min[0]:vox_max[0], vox_min[1]:vox_max[1], vox_min[2]:vox_max[2]] = 1

            model.density.grid = nn.Parameter(model.density.grid * bbox_mask)
        ...
        render_viewpoints_kwargs = {
            ...
            'render_kwargs': {
                ...
                'flip_y': cfg.data.flip_y,
                'render_depth': True,
                'is_eval': args.render_test,
                'bg_color': cfg.data.bg_color,
                'white_bkgd': cfg.data.white_bkgd,
                'render_depth_sigma_thres': cfg.data.render_depth_sigma_thres,
            },
        }
    ...
    if args.render_test:
        ...
        imageio.mimwrite(os.path.join(testsavedir, 'video.depth.mp4'), utils.to8b(depth_vis), fps=3, quality=8)
        imageio.mimwrite(os.path.join(testsavedir, 'video.rgb.mp4'), utils.to8b(rgbs), fps=30, quality=8)
        # imageio.mimwrite(os.path.join(testsavedir, 'video.depth.mp4'), utils.to8b(1 - depths / np.max(depths)), fps=30, quality=8)
